...
from vis_helper import _finish, _titled, _category_colors

_NEUTRAL = "#b8b7b1"      # a rule that did not pass the cutoff
_HAIRLINE = "#c3c2b7"     # the cutoff lines
_ZERO = "#8d8c85"         # zero: the border between avoiding and attracting, so it shows
_INK = "#52514e"          # every piece of text that is not a title
...
def _spread(values, min_gap, lo, hi):
    ...
    placed = values.copy()
    for lower, upper in zip(np.argsort(placed), np.argsort(placed)[1:]):
        placed[upper] = max(placed[upper], placed[lower] + min_gap)
    placed -= placed.mean() - values.mean()          # keep the block where the lines are
...
def _trend_panel(ax, nets, names, slopes, steady, title, stages, unit="", colors=None):
    ...
    rows = _spread(nets[:, 0], (high - low + 2 * pad) * 0.075, low - pad * 0.6, high + pad * 0.6)
...
def _plain(ax):
    ...
    ax.grid(axis="y", color="#eceae2", lw=0.9)
    ax.set_axisbelow(True)
    ax.spines[:].set_visible(False)              # the grid carries the scale on its own
