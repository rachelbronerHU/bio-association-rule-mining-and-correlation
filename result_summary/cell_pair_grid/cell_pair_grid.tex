...
\begin{figure}[H]
    ...
    A dot means that rule was mined, and \textbf{a heavy black outline means the mirror
    rule was not} --- it fired one way round only. Colour is log2 lift, so $+1$ and $-1$ sit
    equally far from white. Dot size is confidence. Here: one healthy duodenal FOV
    (Control\_01\_FOV\_1), axes trimmed to the cell types it uses.}
    \label{fig:fov1}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Healthy colon, the 31 cell types folded into their lineages; Muscle and
    Brunner glands keep their own row and column, forming a lineage with nothing else.
    A box averages the log2 lift of every rule inside it, \textbf{weighted by how many
    FOVs each rule appears in} --- otherwise a rule seen in one FOV would count as much as
    one seen in nearly all of them.}
    \label{fig:layers1}
\end{figure}

\textbf{The same picture repeats in both organs and at every stage.} Each lineage sits with
itself --- epithelium 1.48--2.00, stroma 1.37--2.10, immune 1.03--1.33 --- while epithelium
keeps apart from immune cells (0.31--0.52) and from stroma (0.26--0.41), and epithelium
against muscle stays at 0.03--0.09. This is the stereotypic
